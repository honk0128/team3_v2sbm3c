...
public class AnswerProc implements AnswerProcInter {
  ...
  public int list_all_search_count(String word) {
    int cnt = this.answerDAO.list_all_search_count(word);
    return cnt;
  }
  
  @Override
  public int list_search_count(Map <String, Object> map) {
    int cnt = this.answerDAO.list_search_count(map);
    return cnt;
  }
 ...
 // 1 페이지 시작 rownum: now_page = 1, (1 - 1) * 10 --> 0
    ...
    ArrayList<AnswerVO> list = this.answerDAO.list_all_search_paging(map);
    return list;
  }
  
  @Override
  public ArrayList<AnswerVO> list_search_paging(int accountno, String word, int now_page, int record_per_page) {
 // 1 페이지 시작 rownum: now_page = 1, (1 - 1) * 10 --> 0
    int begin_of_page = (now_page - 1) * record_per_page;
    // 시작 rownum 결정
    int start_num = begin_of_page + 1;
    ...
    ArrayList<AnswerVO> list = this.answerDAO.list_search_paging(map);
    return list;
  }
...
//    str.append("<div id='paging'>");
  ...
  @Override
  public String userpagingBox(int movieno, int now_page, String word, String list_file, int search_count,
      int record_per_page, int page_per_blocK) {
