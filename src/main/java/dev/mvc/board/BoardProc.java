...
public class BoardProc implements BoardProcInter {
  ...
  @Override
  public ArrayList<BoardVO> list_cno(int crudcrudcateno) {
    ArrayList<BoardVO> list = this.boardDAO.list_cno(crudcrudcateno);
    return list;
  }
  ...
  @Override
  public String pagingBox(int crudcrudcateno, int now_page, String word, String list_file, int search_count, int record_per_page,
      int page_per_block) {
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성
      str.append("<span class='span_box_1'><A href='" + list_file + "?crudcrudcateno="+crudcrudcateno+"&word=" + word + "&now_page=" + _now_page
          + "'>이전</A></span>");
    }
    ...
    for (int i = start_page; i <= end_page; i++) {
      ...
      if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='" + list_file + "?crudcrudcateno="+crudcrudcateno +"&word=" + word + "&now_page=" + i + "'>" + i
            + "</A></span>");
      }
    }
  ...
  public int list_all_sc(String word) {
      Integer count = this.boardDAO.list_all_sc(word);
