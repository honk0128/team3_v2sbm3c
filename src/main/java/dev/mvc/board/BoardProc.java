...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성
      str.append("<span class='span_box_1'><A href='" + list_file + "?crudcateno="+crudcateno+"&word=" + word + "&now_page=" + _now_page
          + "'>이전</A></span>");
    }
    ...
    for (int i = start_page; i <= end_page; i++) {
      ...
      if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='" + list_file + "?crudcateno="+crudcateno +"&word=" + word + "&now_page=" + i + "'>" + i
            + "</A></span>");
      }
    }
  ...
  // CSS 스타일 추가
  str.append("<style type='text/css'>");
  str.append("#paging_all {text-align: center; margin-top: 5px; font-size: 1em;}");
  str.append("#paging_all A:link {text-decoration:none; color:black; font-size: 1em;}");
  str.append("#paging_all A:hover {text-decoration:none; background-color: #FFFFFF; color:black; font-size: 1em;}");
  str.append("#paging_all A:visited {text-decoration:none;color:black; font-size: 1em;}");
  str.append(".span_box {text-align: center; font-size: 1em; border: 1px solid #cccccc; padding:1px 6px; margin:1px 2px;}");
  str.append(".current_page {background-color: #668db4; color: #FFFFFF;}");
  str.append("</style>");
  
  // 페이징 박스 생성 시작
  str.append("<div id='paging_all'>");
  
  // 이전 그룹으로 이동하는 링크 생성
  if (now_grp > 1) {
  int prev_page = (now_grp - 2) * page_per_block + 1;
  str.append("<span class='span_box'><a href='" + list_file + "?word=" + word + "&now_page=" + prev_page + "'>이전</a></span>");
  }
  ...
  // 다른 페이지에 대한 링크 생성
  str.append("<span class='span_box'><a href='" + list_file + "?word=" + word + "&now_page=" + i + "'>" + i + "</a></span>");
  }
  }
  ...
  str.append("<span class='span_box'><a href='" + list_file + "?word=" + word + "&now_page=" + next_page + "'>다음</a></span>");
  }
  ...
  public int list_all_sc(HashMap<String, Object> hashMap) {
    int cnt = this.boardDAO.list_all_sc(hashMap);
    return cnt;
  }
